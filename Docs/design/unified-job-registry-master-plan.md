...
**A4 遗留切片**：~~A4.5（F9 MainViewModel 拆分）推迟到实机验证后。~~ 第一阶段已完成（见下表 A4.5 行）。F9 第二阶段（连接生命周期三份复刻收拢为一份）仍待实机验证后另行切片。已知轻微项：末项在提交即被拒/幂等命中时，批次收尾最多晚一拍（≤10s 节拍兜底，事件无触发源），可接受。
