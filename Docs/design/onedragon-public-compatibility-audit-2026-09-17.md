...
> 审计日期：2026-09-17。状态：**审计与计划已完成，等待用户审查并明确下令；R0–R6 产品实施尚未开始**。
...
3. 同一个增强流程要么提交完整原生链作为一个节点，要么按不可变快照展开叶子节点；不得先跑完整原生链再跑展开项。首批迁移使用展开方式，才能在原位置安插旧茶包增强动作。
...
| F09 | P1 | `BgiTaskCoordinator` ext 泵；`TaskRunner` 非阻塞抢锁；`PreemptionGate` 来源判断 | 登记父子作业没有自动把所有入口串行化；简单整龙持实际锁会与子锁死锁，IPC 父项内部来源又可能被视为外人 | R2/R5 同一 admission 仲裁和逻辑租约；owner/epoch 传递到叶子 |
...
| F12 | P1 | `ScriptControlViewModel:740–915`；VM 配置 setter/SaveConfig | 重命名/删除构造 VM，反射修改 DomainName 和 tuple 并写回；新标准格式会漏引用或走旧迁移 | R2 引用服务，R3 全部旧格式写入者退出 |
...
| R2 兼容桥和执行事实 | R0/R1 数据合同 | BGI+助手+服务器/网页 taskId/revision/config-applied 适配；运行上下文与类型化结果；引用服务；幂等/批次跳过修正 | F01–F07、F09/F12 对应入口用集成/故障注入验证；旧格式仍可读，迁移未激活 |
...
| R4 槲寄生承接旧增强 | R2、R3 | WorkflowStore/RunStore/Planner/Reconciler+最低可用任务中心；全部旧计划/循环/账号/自定义秘境/树脂/收尾节点；启动中心真实移交 | 迁移样例从准备到最终动作纯本机跑通；等待不占槽；重复节点、停止/失联/恢复语义完整 |
...
- [ ] R2.1 建稳定 ID/旧键兼容 DTO，同时更新 BGI handlers、助手展示/命令、服务端缓存、网页。
- [ ] R2.2 配置应用回执与开始条件接通；在飞写入超时明确失败；覆盖远端缓存旧命令。
- [ ] R2.3 建运行上下文及 resume 提交回执，修复缺配置/拒绝/异常假成功；父子关系不由 UI 推断。
- [ ] R2.4 修幂等键、批次跳过顺序与引用更新，确保同名重复和父子同名不被去重。
...
- [ ] R4.2 计划排序/星期/定时/循环/跨天、账号、兑换码、自定义秘境、树脂和完成后动作全部具备承接能力；不能以只支持“运行整龙”作为迁移完成。
