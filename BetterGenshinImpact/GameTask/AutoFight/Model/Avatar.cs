    ...
    public void Attack(int ms = 0)
{
    ...
    /// <summary>
    /// 判断当前角色是否处于玛薇卡摩托状态（薄转发：实现已收拢至 AvatarCombatSpecialization，
    /// 色差版当前全仓零调用，保留实例入口以兼容潜在的策略脚本反射调用）。
    /// </summary>
    public bool IsMotorcycle() => AvatarCombatSpecialization.IsMotorcycle(Name);
    ...
    public void UseSkill(bool hold = false,int retryTimes = 1)
    {
        for (var i = 0; i < retryTimes; i++)
        {
            ...
            if (hold)
            {
                ...
                {
                    // 恰斯卡特化逻辑（角色特化域 → AvatarCombatSpecialization，飞行旋转搜索整块收拢）
                    AvatarCombatSpecialization.RunChascaFlightRotation(Ct);
                }
                else
                {
    ...
    public void Charge(int ms = 0)
    {
        ...
        {
            // 桑多涅重击特化逻辑（角色特化域 → AvatarCombatSpecialization）
            AvatarCombatSpecialization.ChargeSandrone(Ct, ms);
        }
        else
        {
            Simulation.SendInput.SimulateAction(GIActions.NormalAttack, KeyType.KeyDown);
