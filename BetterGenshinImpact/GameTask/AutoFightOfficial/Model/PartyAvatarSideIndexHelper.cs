...
public class PartyAvatarSideIndexHelper
{
    ...
    public static MultiGameStatus DetectedMultiGameStatus(ImageRegion imageRegion, AutoFightAssets? autoFightAssets = null, ILogger? logger = null)
    {
        ...
        // 判断当前联机人数
        var pRaList = imageRegion.FindMulti(RecognitionAssets.Get("AutoFightOfficial", "P", imageRegion));
        if (pRaList.Count > 0)
        {
            ...
            // 联机状态下判断
            var onePRa = imageRegion.Find(RecognitionAssets.Get("AutoFightOfficial", "OneP", imageRegion));
            if (onePRa.IsExist())
            {
