...
using BetterGenshinImpact.GameTask.AutoFightOfficial.Config;
...
namespace BetterGenshinImpact.GameTask.AutoFightOfficial.Model;
...
public class CombatScenes : IDisposable
{
    ...
    public CombatScenes InitializeTeam(ImageRegion imageRegion, AutoFightOfficialConfig? autoFightConfig = null)
    {
        if (autoFightConfig == null)
        {
            autoFightConfig = TaskContext.Instance().Config.AutoFightOfficialConfig;
        }
    ...
    /// 静默模式下的队伍识别（用于后台挂件）
    /// </summary>
    public CombatScenes InitializeTeamSilent(ImageRegion imageRegion, AutoFightOfficialConfig? autoFightConfig = null)
    {
        try
        {
            if (autoFightConfig == null)
    ...
    private void InitializeTeamFromConfig(string teamNames, AutoFightOfficialConfig autoFightConfig)
    {
    ...
    private Avatar[] BuildAvatars(List<string> names, List<Rect>? nameRects = null, List<Rect>? avatarIndexRectList = null, AutoFightOfficialConfig? autoFightConfig = null)
    {
        if (autoFightConfig == null)
        {
            autoFightConfig = TaskContext.Instance().Config.AutoFightOfficialConfig;
    ...
    {
        // 优先取配置
        if (!string.IsNullOrEmpty(TaskContext.Instance().Config.AutoFightOfficialConfig.TeamNames))
        {
            InitializeTeamFromConfig(TaskContext.Instance().Config.AutoFightOfficialConfig.TeamNames, TaskContext.Instance().Config.AutoFightOfficialConfig);
            return this;
        }
    ...
    private void ParseTeamOcrResult(OcrResult result, ImageRegion rectArea)
    {
        ...
        if (names.Count == 3)
        {
            // 流浪者特殊处理
            // 4人以上的队伍，不支持流浪者的识别
            var wanderer = rectArea.Find(RecognitionAssets.Get("AutoFightOfficial", "WandererIcon", rectArea));
            if (wanderer.IsEmpty())
            {
                wanderer = rectArea.Find(RecognitionAssets.Get("AutoFightOfficial", "WandererIconNoActive", rectArea));
            }
