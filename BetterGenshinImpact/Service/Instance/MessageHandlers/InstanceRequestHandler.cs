    ...
    private async Task<InstanceIpcEnvelope> HandleTaskStart(InstanceConnection connection, InstanceIpcEnvelope request)
    {
        try
        {
            ...
            await Application.Current?.Dispatcher.InvokeAsync(async () =>
                {
                    var cancellationContext = BetterGenshinImpact.Core.Script.CancellationContext.Instance;
                    _logger.LogInformation("[IPC task.start] group={Group} 停止前 WasCancelled={W}", groupName, cancellationContext.WasCancelled);
                    cancellationContext.Cancel();
                    _logger.LogInformation("[IPC task.start] group={Group} Cancel()后 WasCancelled={W} IsDisp={D} CancelReq={R}",
                        groupName, cancellationContext.WasCancelled, cancellationContext.IsDisposed, cancellationContext.IsCancellationRequested);
                })!;
                ...
                if (groupJson != null)
                {
                    ...
                    await Application.Current!.Dispatcher.InvokeAsync(async () =>
                    {
                        try
                        {
                            ...
                            var preRunCtx = BetterGenshinImpact.Core.Script.CancellationContext.Instance;
                            _logger.LogInformation("[IPC task.start] group={Group} RunMulti执行前 WasCancelled={W} IsDisp={D} CancelReq={R}",
                                groupName, preRunCtx.WasCancelled, preRunCtx.IsDisposed, preRunCtx.IsCancellationRequested);
