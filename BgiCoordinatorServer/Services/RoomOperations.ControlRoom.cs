...
public sealed partial class RoomOperations
{
    ...
    public async Task SendRemoteCommandAsync(GatewayHandlerContext ctx, RemoteCommand command)
    {
        try
        {
            ...
            // [监控模式修复] 回复命令（remote_config.data / push_result、task_policy.data / push_result、
            // schedule_list.data）额外投递给目标 UID 的遥控端连接：监控端不入 _controlRooms、
            // ResolveTargets 匹配不到，回复会被误判"目标离线"进缓存，导致监控端发起拉取/编辑永远
            // 等不到回包（切回执行模式冲刷缓存才恢复）。task_policy.* / schedule_list.data 与
            // remote_config.* 同模式（助手 TaskPolicySyncService 按 CommandId 关联 TCS 等待），同病同药。
            // 回复命令按 CommandId 关联、只有发起方的等待会话会消费，同 UID 执行端收到只会忽略，双投安全。
            // 执行类命令维持 FR-3 不变：遥控端一律不接收。
            var isReplyCommand = command.Cmd
                is "remote_config.data" or "remote_config.push_result"
                or "task_policy.data" or "task_policy.push_result"
                or "schedule_list.data";
            if (isReplyCommand)
            {
